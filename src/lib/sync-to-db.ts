...
import pLimit from 'p-limit'; // this will limit the number of concurrent requests so that we don't overload the API (p-limit = Promise Limiter)
...
export async function syncEmailsToDatabase(emails : EmailMessage[], accountId : string) {
    ...
    const limit = pLimit(10); // limit the number of concurrent requests to 10 req per batch
    try{
...
async function upsertEmail(email : EmailMessage, accountId : string, index : number) {
    ...
    try{ 
        ...
        // email => [name : string, address : string] we are storing email in this format.It represents a map of address to name. (Basically, address in the for loop is this )
        ...
        // here we are storing the promise returned by upsertEmailAddress function in this list.
